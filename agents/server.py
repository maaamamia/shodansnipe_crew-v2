...
# ── STARTUP: restore engine from persisted key ──────────────────────────
...
async def scope_set_alias(body: dict = Body(...)) -> dict:
    ...
    return await set_scope(scope_body)
...
def crew_run(body: dict = Body(...)) -> dict:
    """Trigger a crew run — returns immediately, crew runs async in Terminal 2."""
    scope  = (body.get("scope") or "").strip()
    mode   = (body.get("mode") or "hitl").strip()
    # Store the run request in config so poc_crew.py can poll it if needed
    ...
    audit("crew_run_triggered", {"scope": scope, "mode": mode})
    return {"status": "queued", "scope": scope, "mode": mode,
            "note": "Run crewai.bat in Terminal 2 to execute"}
