...
STEP 4 — ALSO FLAG (without a CVE ID):
  Even if Shodan did not flag a CVE, flag these service exposures:
  - Any host on port 23 (Telnet — cleartext, always Critical)
  - Any host on port 2375 (Docker API — unauthenticated, always Critical)
  - Any host on port 6379 with no auth (Redis — always High)
  - Any host on port 27017 with no auth (MongoDB — always High)
  - Any host on port 9200 with no auth (Elasticsearch — always High)
  - Any host on port 6443 (Kubernetes API — always High)
  - Expired SSL certs — always Medium
  - Missing DMARC/SPF — always Medium
...
LAYER 2 — DEEP ENRICHMENT (Critical and High hosts only):
  ...
  b) wayback_lookup("<hostname or ip>", check_sensitive_paths=True)
