...
class RedTeamChainTool(BaseTool):
    ...
    def _run(self, findings: str, scope: str = "") -> str:
        ...
        for finding in (data if isinstance(data, list) else [data])[:10]:
...
class IOCGeneratorTool(BaseTool):
    ...
    def _run(self, hosts: list[str], hostnames: list[str] = None, ports: list[int] = None) -> str:
        ...
        iocs = {
            "ip_watchlist": [{"ip": ip, "type": "exposed_attack_surface", "confidence": "high"} for ip in hosts[:20]],
            "hostname_watchlist": [{"hostname": h, "type": "exposed_service_dns"} for h in hostnames[:10]],
            "siem_rules": [],
            "firewall_blocks": [],
            "sigma_skeletons": [],
        }
...
def build_threat_intel_agent(llm) -> Agent:
    return Agent(
        ...
        backstory=(
            ...
            "every finding gets a concrete attack path and a concrete remediation."
        ),
        tools=[
            MitreAttackTool(),
            ThreatActorTool(),
...
INPUT DATA:
Vulnerability findings: {vuln_output[:3000] if vuln_output else 'pull from get_results tool'}
Recon findings: {recon_output[:2000] if recon_output else 'use all prior agent outputs'}

STEPS:
1. EXTRACT CRITICAL/HIGH HOSTS: From vuln + recon output, list all Critical and High risk hosts
   with their exposed services, ports, and CVEs.
...
{{
  "threat_intel_report": {{
    ...
    "immediate_actions": ["patch X", "block port Y", ...]
  }},
...
""",
        expected_output=(
            "JSON with threat_intel_report (MITRE mapping, actor attribution, IOCs, immediate actions) "
            "and red_team_recommendations (attack chains, pentest scope, blast radius assessment)"
        ),
        agent=agent,
    )
