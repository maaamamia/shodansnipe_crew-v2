        ...
        try:
            ...
            try:
                ...
                for cdn, hdr in cdn_signals.items():
                    if hdr.lower() in hdrs_lower:
                        ...
                        break
                if not cdn_detected:
                    tags.append({"id": "NO_WAF", "label": "No WAF/CDN detected",
            ...
            try:
                ...
                try:
                    ...
                    dns_resolves = False
                    tags.append({"id": "DANGLING_DNS", "label": "DNS does not resolve",
...
STEP 2 — SEVERITY TRIAGE:
  ...
  CRITICAL: path returning 200 + sensitive file (/.env, /.git/config, /wp-config.php)
  CRITICAL: JSON keyword scan hit (token/secret/api_key found in response body)
  CRITICAL: No auth on admin interface
  HIGH: Swagger/GraphQL/API docs exposed without auth
  HIGH: Basic Auth (cleartext credentials)
  HIGH: CORS open (Access-Control-Allow-Origin: *)
  HIGH: POTENTIAL_TAKEOVER posture tag
  MEDIUM: Missing security headers (HSTS, CSP, X-Frame-Options)
  MEDIUM: DANGLING_DNS posture tag
