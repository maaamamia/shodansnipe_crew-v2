...
def expand_queries(org: str = "", domain: str = "", cidrs: list[str] | None = None,
    ...
    """Generate a broad, de-duplicated Shodan query set from scope + observed data.

    Pulls org-parameterised templates from query_advisor (attack-surface, misconfig,
    modern-infra) when importable, and always adds the high-value creative pivots so the
    package is wide even if the template catalogue isn't on the path.
    """
    cidrs = cidrs or []
    ...
    def add(q: str, why: str, prio: str = "HIGH"):
        q = (q or "").strip()
        if q and q not in seen and "{" not in q:
            seen.add(q)
    ...
    for c in cidrs[:4]:
        add(f"net:{c}", "sweep confirmed block — surfaces the unknowns", "HIGH")
    for p in products[:6]:
        add(f'product:"{p}"', f"enumerate {p} cohort (version/CVE seeding)", "MEDIUM")

    # Render org/domain-parameterised templates from the advisor catalogue. We ONLY keep
    # templates that are actually scoped to this org/domain (≥1 org/domain param filled), so
    # global threat-hunting queries (Cobalt Strike, mirai, ransomware) and any template needing
    # a param we can't fill (asn/net/ip/port/bucket) are skipped — no garbage like `asn:Dell`
